...
*** Test Cases ***
TST0001: Anexar log de execução no JIRA para o teste que encerra participante Emissor
    [Tags]    UPLOAD_RESULTADO_JIRA
    ${TEST_KEY}=            Set Variable        64135
    ${TEST_EXEC_KEY}=       Set Variable        64139
    ${PATH_EXEC}=           Set Variable        ${CURDIR}\\Resultados\\Emissor
...
TST0002: Anexar log de execução no JIRA - Encerramento de participante Emissor que passou a ser considerado Instituição Financeira
    [Tags]    UPLOAD_RESULTADO_JIRA
    ${TEST_KEY}=            Set Variable        64135
    ${TEST_EXEC_KEY}=       Set Variable        64141
    ${PATH_EXEC}=           Set Variable        ${CURDIR}\\Resultados\\DeixandoDeSerEmissor
    ${NAME_ZIP_IMG}=        Set Variable        imgTelaNoMe.zip
    ${NAME_ZIP_CSV}=        Set Variable        csvFilaBancoDeDados.zip
    ${NAME_LOG_EXEC}=       Set Variable        log.html
    ${DIR_LOG_EXEC}=        Set Variable        ${PATH_EXEC}\\${NAME_LOG_EXEC}
