...
*** Settings ***
Documentation       Encerramento de participante INR.
...
TST0002: Verificar miolo de contas de participante INVESTIDOR NAO RESIDENTE que serão enviados para outros sistemas após efetivação
    [Tags]    PROGRESSIVO_FILA_EVENTO_PARTICIPANTE_ON
    Dado que eu defina a variavel TAG_NUMBER com o valor TST0002 como uma variavel global
    E que eu realize o cadastro de um novo participante INVESTIDOR NAO RESIDENTE utilizando dados do RIOBM para preencher a familia
    E que eu defina a variavel NAME_CSV com o valor TST0002-INVESTIDOR-NAO-RESIDENTE-Efetivacao como uma variavel global
...
TST0003: Encerramento de participante INVESTIDOR NAO RESIDENTE no sistema NoMe
    [Tags]    PROGRESSIVO_FILA_EVENTO_PARTICIPANTE_ON
...
TST0004: Verificar miolo de contas de participante INVESTIDOR NAO RESIDENTE que serão enviados para outros sistemas após encerramento
    [Tags]    PROGRESSIVO_FILA_EVENTO_PARTICIPANTE_ON
#    Set Global Variable     ${CNPJ_PARTICIPANTE}        10.476.258/0001-12

    Dado que eu defina a variavel NAME_CSV com o valor TST0004-INVESTIDOR-NAO-RESIDENTE-Encerramento como uma variavel global
    E que eu execute a query consultando a fila de eventos após encerramento de participante e exporte o resultado para csv
    Quando eu busco os tipos de miolo de conta que estão habilitados na tabela parametro config
    Então eu valido os tipos de contas ${miolo_contas_habilitado} no arquivo ${NAME_CSV} exportado para o diretorio ${PATH_CSV}

TST0005: Verificar que não foi enviado nenhum registro para a fila do extrato
