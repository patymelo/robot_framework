...
TST0000.1: PJ RESIDENTE (API) - Massa Inclusão ITAUBM Dados Maximos e depara no NoMe - DONO
    ...
    [Tags]    POSITIVO_API_1
    ${TAG_NUMBER}=       Set Variable   TST0000.1
    ...
    Set Global Variable  ${resposta}
    #Então a chamada deve ter status 200
TST0001: PJ RESIDENTE (API) - Alteração ITAUBM Dados Minimos - DONO
    ...
    ${DONO}=             Set Variable   SIM
    ${TOGGLE}=           Set Variable   S
    ${VideoAlias}=       Set Variable   AlteracaoApiPJResidente
    ...
    ${resposta}=         Que eu faça uma chamada PUT enviando o ${body_alterado}
    Set Global Variable  ${resposta}
    Então a chamada deve ter status 200

TST0002: PJ RESIDENTE (API) - Alteração ITAUBM Dados Maximos - DONO
    [Documentation]   Teste de API Comitente PJ como Dono do cadastro: Validar sucesso no cadastro com dados máximos preenchidos
    [Tags]    POSITIVO_API_1
...
TST0003: PJ RESIDENTE (API) - Alteração RIOBM Dados minimos - NÃO DONO
    ...
    Setar variaveis comitente PJ residente API
    Dado Que eu chame a api de alteração de comitente PJ residente passando a apiKey RIOAPIINT
    ${body}=        Que eu crie um novo body a partir de um ${JSON_INC_MIN}
    Set Global Variable  ${body}
    ${body_alterado}=    Setar dados no body da API PJ residente
    ${resposta}=         Que eu faça uma chamada PUT enviando o ${body_alterado}
    Set Global Variable  ${resposta}
    Então a chamada deve ter status 200
TST0004: PJ RESIDENTE (API) - Alteração ITAUBM Dados Maximos - DONO
    [Documentation]   Teste de API Comitente PJ como não Dono do cadastro: Validar sucesso no cadastro com dados máximos preenchidos
    [Tags]    POSITIVO API
